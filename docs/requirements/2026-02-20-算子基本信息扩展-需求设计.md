...
| dataFormat | 数据格式 | Checkbox.Group | 是（发布时） | 至少选择一项，值只能是 "1"、"10"、"12" |
| generator | 生成方式 | Select | 否 | 动态/静态 |

**说明：**
- 保存草稿时，dataFormat 可以为空（不验证必填）
- 发布算子时，dataFormat 必须至少选择一项
- dataFormat 的值只能是预定义的枚举值（"1"-"静态MML"、"10"-"动态MML"、"12"-"话统"）
- generator 的值只能是 "dynamic"（动态）或 "static"（静态）
...
- [ ] Data Format 支持多选，发布时至少选择一项
- [ ] Data Format 的值只能是预定义的枚举值（"1"、"10"、"12"）
- [ ] Generator 支持动态/静态单选，可为空
- [ ] Generator 的值只能是 "dynamic" 或 "static"
- [ ] 算子列表正确显示新增字段
- [ ] 算子详情正确显示新增字段
- [ ] 编辑算子时正确回显新增字段
- [ ] 所有新增字段的数据格式正确存储和展示
...
| 功能点 | 设计要求 | 实现情况 | 状态 |
|--------|---------|----------|------|
| 数据库表结构更新 | 新增 4 个字段 | V2__add_operator_fields.sql 已创建 | ✅ 完成 |
| 数据库索引 | 唯一索引 + 普通索引 | 已创建 idx_operator_code, idx_object_code, idx_data_format, idx_generator | ✅ 完成 |
| 枚举定义 | DataFormat 和 Generator 枚举 | 已在 operator-common 模块定义 | ✅ 完成 |
| Operator 实体更新 | 添加 4 个字段 | 已添加，operatorCode 配置了 unique 约束 | ✅ 完成 |
| OperatorDto 更新 | 包含新字段 | OperatorRequest 和 OperatorResponse 已更新 | ✅ 完成 |
| 格式校验注解 | @OperatorCode 注解 | 已创建，使用正则 `^[a-zA-Z_][a-zA-Z0-9_]{0,63}$` | ✅ 完成 |
| 唯一性校验 | operatorCode 全局唯一 | OperatorServiceImpl 中实现 | ✅ 完成 |
| dataFormat 验证 | 只能是 "1"、"10"、"12" | 前端 CheckboxGroup 限制，后端暂无验证 | ⚠️ 待完善 |
| generator 验证 | 只能是 "dynamic"、"static" | 前端 Select 限制，后端暂无验证 | ⚠️ 待完善 |
| 前端表单更新 | 新增 4 个字段 | create.tsx 已更新 | ✅ 完成 |
| 前端列表展示 | 显示新字段 | list.tsx 已更新，支持多选格式展示 | ✅ 完成 |
| 前端详情展示 | 显示新字段 | detail.tsx 已更新 | ✅ 完成 |

### 7.2 待完善项

**后端枚举验证：**
- 当前后端 Service 层直接接收 dataFormat 和 generator 的字符串值，未进行枚举值有效性验证
- 建议在 OperatorServiceImpl 中添加验证逻辑，确保：
  - dataFormat 只能是 "1"、"10"、"12" 或它们的组合（如 "1,10"）
  - generator 只能是 "dynamic" 或 "static"

**实现建议：**
```java
// 在 OperatorServiceImpl 的 createOperator 和 updateOperator 方法中添加验证

// 验证 dataFormat
if (request.getDataFormat() != null && !request.getDataFormat().isEmpty()) {
    String[] formats = request.getDataFormat().split(",");
    for (String format : formats) {
        try {
            DataFormat.fromCode(format.trim());
...
// 验证 generator
if (request.getGenerator() != null && !request.getGenerator().isEmpty()) {
    try {
        Generator.fromCode(request.getGenerator());
    } catch (IllegalArgumentException e) {
        throw new IllegalArgumentException("Invalid generator: " + request.getGenerator());
    }
}
```
...
| v1.1 | 2026-02-20 | 添加实现状态章节，记录完成情况和待完善项；澄清前端表单验证规则（保存草稿 vs 发布） | Claude |
