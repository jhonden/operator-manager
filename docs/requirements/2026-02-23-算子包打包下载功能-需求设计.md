...
| **响应头** | `Content-Disposition: attachment; filename="operator_package_{id}.zip"` |
 ...
 */
export const downloadPackage = async (packageId: number): Promise<void> => {
  try {
    ...
    link.href = url;
    link.setAttribute('download', `operator_package_${packageId}.zip`);
...
| **A7：前端下载** | 点击按钮能正常下载压缩包 | 浏览器中点击下载按钮 |
